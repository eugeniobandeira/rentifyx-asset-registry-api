...
- [ ] A moderation verdict (from the future Kafka consumer) transitions the asset per plan T-077/T-078, refined against `rentifyx-ai-services`' ADR-AI-004 (Approved/Rejected are immediate decisions, only PendingReview needs a human): `Approved` → `Active` (+`AssetPublished`); `Rejected` → `Archived` (terminal, no admin involvement); `PendingReview` → held in `PendingModeration`, no state change, awaiting `AdminReviewAsset`
...
| `Rejected`/`ManualReview` as a new `AssetStatus` | `Rejected` reuses the existing `Archived` status (terminal); `PendingReview` reuses `PendingModeration` — no new domain status needed |
| A "revert to Draft" transition after rejection | Rejected assets are archived (terminal), not returned to the owner for editing — resubmission after rejection is out of scope for this pass |
...
2. WHEN verdict is `Rejected` THEN system SHALL transition the asset to `Archived` (terminal, no admin involvement) — matches `rentifyx-ai-services`' ADR-AI-004, which treats `Rejected` as an immediate decision, not one requiring human review
3. WHEN verdict is `PendingReview` THEN system SHALL leave the asset in `PendingModeration` (no state change) and log the outcome, awaiting `AdminReviewAsset`
4. WHEN the asset is not currently `PendingModeration` (e.g. verdict replayed after already `Active`/`Archived`) THEN system SHALL no-op and return the current state, not error — this is the idempotency guarantee for at-least-once Kafka delivery
5. WHEN the asset does not exist THEN system SHALL return `NotFound`
...
2. WHEN a caller with `IsAdmin = true` rejects a `PendingModeration` asset THEN system SHALL transition it to `Archived` (terminal, mirrors the automated `Rejected` verdict path) and log the reason (reason itself is not persisted — no field exists to store it)
...
- WHEN `ApplyModerationVerdict` receives an unrecognized/future verdict value THEN system SHALL treat it conservatively as "no state change" (same as `PendingReview`) rather than crash — forward-compatible with `rentifyx-ai-services` adding verdict values later
- WHEN `AdminReviewAsset` targets a non-existent asset THEN system SHALL return `NotFound`

## Known Gap (documented, not solved here)

`Rejected`/admin-reject archives the asset with no path back to `Draft` for the owner to fix and
resubmit. Flagged as a deferred idea, not blocking this pass (plan does not call for a revert
transition, and archiving is an intentional terminal state, not a stuck one).
...
| MOD-06 | P1: Rejected verdict → Archived (terminal); PendingReview → no state change | Application | Pending |
...
| MOD-10 | P2: admin reject → Archived (terminal), reason logged | Application | Pending |
...
**Coverage:** 13 total, 13 mapped to tasks (implicit, execution plan below — Tasks phase skipped), 0 unmapped. Status: Verified (2026-07-22, 7 validator tests + 15 handler tests, all green; revised same day after cross-repo alignment analysis against `rentifyx-ai-services`' ADR-AI-004 — `Rejected` moved from no-op to `Archived`, see ADR-AR-008).
...
- [ ] No new `AssetStatus` value introduced — `Rejected` reuses `Archived`, `PendingReview` reuses `PendingModeration`
